...
import { YoshiLogo, FullLogo, ChivasLogo, ChivasShield } from './logos';

interface TicketPreviewProps {
  data: TicketData;
  innerRef: React.RefObject<HTMLDivElement>;
  activeTab?: 'yoshi' | 'chivas';
}

export const TicketPreview: React.FC<TicketPreviewProps> = ({ data, innerRef, activeTab = 'yoshi' }) => {
  const isChivas = activeTab === 'chivas';
  const primaryColor = isChivas ? '#ee2e24' : COLORS.PRIMARY; // Chivas Red
    ...
    >
      ...
      <div className="relative w-full bg-white flex flex-col items-center justify-start border-b border-dashed border-gray-200 px-10 pt-4 pb-4">
        {isChivas ? (
          <div className="flex flex-col items-center">
            <ChivasLogo className="h-20 w-auto object-contain mb-1" />
          </div>
        ) : (
        ...
        <div className="flex flex-col items-center mt-2 space-y-1">
          {data.isTokens && (
            <span className={`text-[22px] font-black uppercase tracking-[0.3em] leading-none ${isChivas ? 'text-[#ee2e24]' : 'text-[#fa005a]'}`}>
              Tokens
            </span>
          )}
          {data.cortesia && (
            <span className={`text-[22px] font-black uppercase tracking-[0.3em] animate-in zoom-in-95 duration-500 leading-none ${isChivas ? 'text-[#ee2e24]' : 'text-[#fa005a]'}`}>
              Cortesia
            </span>
          )}
        </div>
          ...
          >
            <div className="absolute inset-0 opacity-10 flex flex-wrap gap-2 p-1 overflow-hidden rotate-12 scale-150">
              {Array.from({ length: 20 }).map((_, i) => (
                isChivas ? (
                  <span key={i} className="text-[8px] font-black">CHIVAS</span>
                ) : (
            ...
            <div className="relative z-10 flex flex-col items-center justify-center transform -translate-y-[6px]">
              <span className="text-[9px] font-bold uppercase tracking-[0.2em] opacity-80 leading-none mb-1">
                {data.isTokens ? 'Válido para:' : 'Saldo Disponible'}
              </span>
              <span className={`tracking-tighter leading-none ${data.isTokens ? 'font-serif italic text-[24px]' : 'font-normal text-[34px]'} ${data.saldo.length > 15 && !data.isTokens ? 'text-[18px]' : ''}`}>
                {!data.isTokens && '$'}{data.saldo}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-start py-2 px-8 relative">
        ...
        <p className="text-[18px] font-medium text-gray-400 text-center mb-3 max-w-[85%] leading-tight tracking-tight mt-0">
          {isChivas ? 'Presenta este código para realizar tu canjee' : 'Presenta este código QR para pagar en el recinto'}
        </p>
        <div className="w-full aspect-square bg-white rounded-[2rem] shadow-lg border border-gray-100 px-4 flex items-center justify-center relative overflow-hidden">
          {isChivas && (
            <div className="absolute inset-0 opacity-[0.08] flex items-center justify-center rotate-12 scale-110">
               <ChivasShield className="w-full object-contain" />
            </div>
          )}
          
          <QRCodeSVG 
            value={data.codigo || (isChivas ? 'chivas' : 'yoshi')} 
            size={265}
            level="H"
            fgColor={isChivas ? '#002b5c' : '#000000'}
            bgColor="transparent"
            includeMargin={false}
          />
        </div>
        ...
        {data.valido && data.valido.trim() !== '' && (
          <div className="mt-3 px-3 py-1.5 rounded-md bg-gray-100/20 border border-gray-100/50 backdrop-blur-sm">
            <p className="text-[14px] font-bold text-gray-700 text-center uppercase tracking-widest">
              VALIDO EN RECINTO: <span className={isChivas ? 'text-[#ee2e24]' : 'text-[#fa005a]'}>{data.valido}</span>
            </p>
          </div>
        )}
        ...
        {!data.cortesia && !data.isTokens && (
          <div className="mt-auto pt-1 pb-4 w-full border-t border-dashed border-gray-200/30">
            <p className="text-[19px] font-medium text-gray-400 text-center leading-snug tracking-tight">
              ...
              <span className={`text-[18px] font-normal lowercase tracking-widest block ${isChivas ? 'text-[#ee2e24]' : 'text-[#fa005a]'}`}>
